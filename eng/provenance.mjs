...
// Adapted from ArcForges Cloud; source and changes are recorded in cloud-provenance-port-r1.
...
function strings(value, check = text, nonempty = true) {
...
export function hash(data, normalization = "raw") {
...
export function parseDocument(input) {
  ...
  const quoted = () => {
    ...
    while (cursor < source.length) {
      ...
      if (char === "\\") cursor++;
      else if (char === '"') return JSON.parse(source.slice(start, cursor));
    }
...
const document = (root, file) => object(parseDocument(readOwned(root, file)));
...
function allowed(spdx, category, used = true) {
...
export function validateRecord(value) {
  ...
  strings(notice.files, relative, notice.required);
...
export function comparisonCommit(root) {
  let candidate;
...
export function renderNotice(records, active) {
  const lines = [
...
export function auditProvenance(root, options = {}) {
  root = realpathSync(root);
  ...
  assert.equal(inventory.repository, "AI");
