...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "ai-provenance-"));
  ...
  const record = {
    ...
    lifetime: { status: "permanent", owner: "Test owner", removalTrigger: null },
    generation: null,
    review: {
  ...
  return {
    ...
    cleanup: () => {
      ...
      assert(path.basename(root).startsWith("ai-provenance-"));
...
scenario("reject linked paths escaping a repository", (f) => {
  const outside = mkdtempSync(path.join(os.tmpdir(), "ai-provenance-outside-"));
  try {
