...
function fixture() {
  const catalog = JSON.parse(
    readFileSync(new URL("../version-sources.json", import.meta.url), "utf8"),
  );
...
test("npm lock versions retain coordinate identity and exclude the workspace release", () => {
  ...
  const packages = resolveAxes(f.catalog, f.read).PackageVersion;
...
test("dirty, wrong-source, missing and malformed CI inputs fail closed", () => {
  ...
  const env = {
    GITHUB_ACTIONS: "true",
    GITHUB_SHA: commit,
    GITHUB_REPOSITORY: "ArcForges/AI",
    GITHUB_RUN_ID: "123",
