...
The last two commands modify the selected Cloudflare account and consume model usage. Before creating the real smoke instance, model-free Workflow probes wait for the deployed Worker ID, source commit and build version to match the candidate. They use only `{ "kind": "deployment-probe" }`; that path returns identity before any model/tool step, and bootstrap versions reject it before calling AI. Probes are bounded to 30 instances within a five-minute readiness window. A stale completed probe needs a new probe ID because its output cannot change. Each ID is deterministic (`ready-<worker-version>-<attempt>`), so a lost create response can be resumed by reading the same instance.

Only after readiness succeeds does the smoke use the non-sensitive name `ArcForges`. A successful real smoke makes two bounded model calls and one local greeting tool call. It proves that the deployed Workflow, selected model, Chat Completions adapter and published protobuf package interoperate. Readiness alone is not inference evidence. This does not verify product authorization, billing, streaming, R2 or full AI task execution.
...
5. Wait for model-free Workflow readiness, then run the single real Workflow smoke test and publish its evidence. Only a successful live test creates the `ai-0.1.0-ci.<run>.<attempt>` GitHub prerelease with the deployable candidate.
...
- **Workflow readiness times out:** retain the probe records and inspect deployment propagation. No real inference instance was submitted. Run `npm run test:live` again to resume pending probes or use remaining probe IDs; a lost create response is not automatically retried or replaced within a run. If all 30 probes have terminal mismatches, inspect the cause and make a new deployment rather than restarting those immutable instances.
