...
The source/runtime/bundle checks passed before final repository review. That review identified three packaging/tooling corrections: Biome's migration must not disable recommended rules, uncommitted builds must not claim deployable commit provenance, and the protobuf runtime's BSD-3-Clause notice must accompany the Apache notices. These corrections are implemented together with a supported-major constraint for Vitest updates.

After account setup, live checks identified the Free-plan CPU configuration restriction and two provider request-profile differences. The fixes use plan defaults and the verified Chat Completions tool exchange, with regression tests and documentation updated together. The real deployed Workflow passed on 2026-09-14. [Validation evidence](validation.md) records the precise source/version and distinguishes this local OAuth verification from the remaining first main CI deployment using the GitHub token. No additional product scope was added.
