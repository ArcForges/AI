...
| GitHub CI                     | [Current PR checks](https://github.com/ArcForges/AI/pull/1/checks) must pass Verify before merge: Linux/Windows, both CodeQL languages, dependency/secret checks and the compiled candidate. The earlier bootstrap passed [run 34856101234](https://github.com/ArcForges/AI/actions/runs/34856101234) |
...
The original candidate, manifest, deployment record and `live-evidence.json` remain under this worktree's ignored `artifacts/` directory. The tracked record excludes account credentials, email and model reasoning. It records a real deployed Workflow, separately from diagnostic REST calls and local fixtures.
...
PR checks use no Cloudflare secret. Local live verification used OAuth; it cannot validate the separately stored GitHub API token. The first main merge must deploy its own verified candidate, pass the real Workflow gate using that token, and create the GitHub prerelease. A missing/invalid credential or failed inference must leave that job failed.
